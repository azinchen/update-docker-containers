...
* **Standalone containers**: For each running container not managed by Compose, the script pulls the latest image. If a newer image is available, the container is stopped, renamed to `<name>-old`, and recreated with the new image, preserving its command, environment variables, port bindings, volumes, network mode, restart policy, and extra hosts. The old container is removed only after the replacement starts; if the recreate fails, the old container is renamed back and restarted.
